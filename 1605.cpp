...
void sort() {
    ...
    vector<int> rank(L, 0); // buf[i] ~ buf[i+k-1] 로 결정되는 rank 를 저장하는 배열, 길이 L, i - substring : rank 
    for(int i=0;i<L;i++) rank[i] = buf[i] - 'a', sa[i] = i; // initial rank; 
...
void fillLCP() { 
    ...
    for(int i=0;i<L;i++) {
        ...
        int j = sa[r+1]; 
        while(i + h < L && j + h < L && buf[i + h] == buf[j + h]) h++; 
        lcp[r] = h; 
        if (h) h--; 
    }
}
...
int main() {
    ...
    for(int a : lcp) maxLCP = max(maxLCP, a); 
